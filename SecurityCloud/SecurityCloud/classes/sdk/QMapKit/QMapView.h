 ...
 * @brief 设置地图比例尺偏移
 *
 * @param offset 比例尺中心的偏移量. 如果offset为CGPointZeor则为默认位置
 */
- (void)setScaleViewOffSet:(CGPoint)offset;
/**
 * @brief 隐藏默认的用户位置图标
 *
 * @param hide YES则不显示当前位置图标，NO则显示
 */
- (void)setLocationIconHidden:(BOOL)hide;
...
/**
 * @brief 添加annotation，本标注为opengl绘制且无须实现viewforannotation的delegate
 */
- (void)addIconAnnotation:(QIconAnnotation*)anno;

/**
 * @brief 移除annotation，本标注为opengl绘制且无须实现viewforannotation的delegate
 */
- (void)removeIconAnnotation:(QIconAnnotation*)anno;
