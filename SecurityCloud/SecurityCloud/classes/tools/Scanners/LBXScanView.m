...
- (void)startDeviceReadyingWithText:(NSString*)text
{
    ...
    //扫码区域Y轴最小坐标
    CGFloat YMinRetangle = self.frame.size.height / 2.0 - sizeRetangle.height/2.0 - 0;
...
- (void)startScanAnimation
{
    ...
    if (!_scanNetAnimation){
        ...
        [_scanNetAnimation startAnimatingWithRect:_scanRetangleRect
                                           InView:self
                                            Image:[UIImage imageNamed:@"qrcode_scan_full_net"]];
...
- (void)drawScanRect
{
    ...
    //非扫码区域半透明
    {
        ...
        const CGFloat *components = CGColorGetComponents([UIColor colorWithRed:0.0/255 green:0.0/255 blue:0.0/255 alpha:0.5].CGColor);
...
+ (CGRect)getScanRectWithPreView:(UIView*)view
{
    ...
    //扫码区域Y轴最小坐标
    CGFloat YMinRetangle = view.frame.size.height / 2.0 - sizeRetangle.height/2.0 - 0;
    //扫码区域坐标
