 ...
 */
+ (UIImage *)SG_generateWithLogoQRCodeData:(NSString *)data logoImageName:(id)logoImageName logoScaleToSuperView:(CGFloat)logoScaleToSuperView {
    // 1、创建滤镜对象
    CIFilter *filter = [CIFilter filterWithName:@"CIQRCodeGenerator"];
    ...
    if ([logoImageName isKindOfClass:[UIImage class]]) {
        icon_image = logoImageName;
    }else{
        NSString *icon_imageName = logoImageName;
        icon_image = [UIImage imageNamed:icon_imageName];
    }
